...
if len(sys.argv) == 2:
	...
	if sys.argv[1] == 'commiting':
		rest = shell_exec('git add .; git commit -m "fixed"; git push -u origin main;')
		mKey = ''
...
def remIgnores(cdc):
	rmItem(cdc+'/.git')
	rmItem(cdc+'/node_modules')		
	if False:
		rmItem(os.path.abspath(cdc+'/'+json.loads(shell_exec('cat tsconfig.json'))['compilerOptions']['outDir']))
		rmItem(os.path.abspath(cdc+'/'+json.loads(shells_exec('cat tsconfig.json'))['compilerOptions']['outDir']))
		...
		if len(list_) > no and list_[no].find(' ') > -1:
			...
			if len(afile) > 0:
				...
				if checksum_name == afile:
					...
					remIgnores(compare_dir+"/"+str(cnt))
